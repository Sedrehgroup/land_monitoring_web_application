...
import utils
from torch_semi_seg_model.furnace.utils.pyt_utils import load_model
    ...
    def __init__(self, block, layers, norm_layer=nn.BatchNorm2d, bn_eps=1e-5,
        ...
        if deep_stem:
            self.conv1 = nn.Sequential(
                nn.Conv2d(in_channels=16, out_channels=13, kernel_size=(1, 1), stride=2, padding=1,
                          bias=False),
                nn.Conv2d(in_channels=13, out_channels=stem_width, kernel_size=3, stride=2, padding=1,
                          bias=False),
