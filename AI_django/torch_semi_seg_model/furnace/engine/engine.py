...
# @File    : engine.py
# To test GPUs: ===> $ ./build/all_reduce_perf -b 8 -e 128M -f 1 -g 2 <====
import os
...
os.environ['MASTER_PORT'] = '16001'
os.environ['WORLD_SIZE'] = '1'
# os.environ['NCCL_SOCKET_IFNAME']= '8010'
class State(object):
    def __init__(self):
        self.epoch = 0
        self.iteration = 0
...
class Engine(object):
    def __init__(self, custom_parser=None):
        ...
        logger.info(
            "PyTorch Version {}, Furnace Version {}".format(torch.__version__,
                                                            self.version))
        self.state = State()
        self.devices = list(range(torch.cuda.device_count())) #None
        ...
        if self.distributed:
            ...
            os.environ['MASTER_PORT'] = '16001'
            dist.init_process_group(backend="nccl", init_method="env://", world_size=self.world_size, rank= self.args.local_rank)
    ...
    #'--devices', default = list(range(torch.cuda.device_count())
    def inject_default_parser(self):
        p = self.parser
        p.add_argument('-d', '--devices', default= list(range(torch.cuda.device_count())),
