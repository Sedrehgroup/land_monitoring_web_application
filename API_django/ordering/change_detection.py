...
class Change:
    def __init__(self, polygon , geom_id, before_name, after_name, path):
    ...
    def change_analysis(self):
        ...
        coords = Polygon( json.loads(str(self.polygon).replace("\'", "\""))['features'][0]['geometry']['coordinates'][0])

        before_raster , something = mask(dataset=self.before_dataset, shapes=[coords], crop=True)
        after_raster , something = mask(dataset=self.after_dataset, shapes=[coords], crop=True)
